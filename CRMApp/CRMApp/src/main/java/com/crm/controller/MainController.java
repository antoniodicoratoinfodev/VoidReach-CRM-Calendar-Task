...
public class MainController {
    ...
    public void initialize() {
        searchField.textProperty().addListener((obs, old, newValue) -> filterContacts(newValue));
        setupColumns();
    ...
    private void loadDummyContacts() {
        ...
        filteredData.addAll(contactData); contactsTable.setItems(filteredData);
    }
    ...
    private void updateRightSidebar() {
        ...
        for (int i = 0; i < daysInMonth; i++) {
            ...
            if (date.equals(calendarDatePicker.getValue())) dBtn.setStyle("-fx-background-color: #3b82f6; -fx-text-fill: white; -fx-background-radius: 50%;");
            else if (date.equals(LocalDate.now())) dBtn.setStyle("-fx-border-color: #3b82f6; -fx-border-radius: 50%;");
        ...
        if (tasks.isEmpty()) {
            ...
            noTasks.setStyle("-fx-text-fill: #94a3b8; -fx-font-style: italic; -fx-padding: 10;");
            ...
            for (Task t : tasks) {
                ...
                item.setOnMouseClicked(e -> handleNavigationToCalendar());
    ...
    private void showContactDialog(Contact contact) {
        ...
        result.ifPresent(c -> { if (contact == null) { contactData.add(0, c); filterContacts(searchField.getText()); } contactsTable.refresh(); });
    }

    private void filterContacts(String searchText) {
        if (searchText == null || searchText.isEmpty()) filteredData.setAll(contactData);
        else {
            String lcf = searchText.toLowerCase();
            filteredData.setAll(contactData.filtered(c -> 
